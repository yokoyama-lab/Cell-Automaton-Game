...
// ===== ゲーム本体 =====
class Game {
  private grid: number[][];   // 現在の生存状態 isLiving (0..4)
  private next: number[][];   // 次世代 willLiving
  score = 0;
  highScore = 0;
  level = 1;
  comboMult = 1;
  placeColor = 1;             // 配置するセルの色 (1..4)
  running = false;
  gameover = false;
  living = 0;
  ...
  private static makeGrid(): number[][] {
    const g: number[][] = [];
  ...
  private static isCorner(x: number, y: number): boolean {
    return (x === 0 || x === COLS - 1) && (y === 0 || y === ROWS - 1);
  }

  private static isBorder(x: number, y: number): boolean {
    return x === 0 || x === COLS - 1 || y === 0 || y === ROWS - 1;
  }
  ...
  step(): void {
    ...
    for (let y = 0; y < ROWS; y++) {
      for (let x = 0; x < COLS; x++) {
        ...
        if (self === 0) {
          ...
          // --- 死亡判定：同色の上下左右隣接で決まる ---
          const same = (dx: number, dy: number) => this.at(x + dx, y + dy) === self;
          const up = same(0, -1);
          ...
          if (cnt >= 2) {
            ...
            if (up && (same(-1, -1) || same(0, -2) || same(1, -1))) dies = true;
            else if (left && (same(-1, -1) || same(-2, 0) || same(-1, 1))) dies = true;
            else if (right && (same(1, -1) || same(2, 0) || same(1, 1))) dies = true;
            else if (down && (same(-1, 1) || same(1, 1) || same(0, 2))) dies = true;
          }
    ...
    for (let y = 0; y < ROWS; y++) {
      for (let x = 0; x < COLS; x++) {
        if (this.grid[y][x] > 0) this.living++;
      }
    }
  ...
  spinCells(x: number, y: number): void {
    if (this.gameover) return;
    // 端と最後の2列/2行は操作不可（Java 版と同じ可動域 x,y ∈ 1..11）
    if (!(x >= 1 && y >= 1 && x < COLS - 2 && y < ROWS - 2)) return;

    const g = this.grid;
    const empty =
      g[y][x] === 0 && g[y + 1][x] === 0 && g[y][x + 1] === 0 && g[y + 1][x + 1] === 0;
  ...
  cellAt(y: number, x: number): number {
...
class App {
  ...
  private statusTop: HTMLElement;
  private statusBottom: HTMLElement;
  ...
  constructor() {
    ...
    this.colorBtn = document.getElementById("colorBtn") as HTMLButtonElement;
